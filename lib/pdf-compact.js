...
export function makeUltraCompact(html) {
  ...
  let changes = { fontSize: 0, padding: 0, margin: 0, lineHeight: 0, photo: 0 }
  ...
  // 2. FONT-SIZE: Escala agresiva - INCLUIR CASE INSENSITIVE Y DECIMALES
  html = html.replace(/font-size:\s*(\d+(?:\.\d+)?)px/gi, (match, size) => {
  ...
  // 3. PADDING: Máximos - MEJORADO con case insensitive
  html = html.replace(/padding:\s*(\d+)px(?:\s+(\d+)px)?/gi, (match, v1, v2) => {
    changes.padding++
  ...
  html = html.replace(/padding-bottom:\s*(\d+)px/gi, (match, size) => {
    changes.padding++
    const num = parseInt(size)
  ...
  html = html.replace(/margin-bottom:\s*(\d+)px/gi, (match, size) => {
    changes.margin++
    const num = parseInt(size)
  ...
  // 5. MARGIN genérico
  html = html.replace(/margin:\s*(\d+)px\s+0/gi, (match, size) => {
    changes.margin++
    const num = parseInt(size)
  ...
  html = html.replace(/margin:\s*0\s+0\s+(\d+)px\s+0/gi, (match, size) => {
    changes.margin++
    const num = parseInt(size)
    const newSize = num >= 10 ? 2 : 1
  ...
  html = html.replace(/line-height:\s*[\d.]+/gi, (match) => {
    changes.lineHeight++
    return 'line-height: 1.0'
  })
  
  // 7. BORDES: Delgados
  html = html.replace(/border(-bottom)?:\s*[234]px/gi, '$1: 1px')
  
  // 8. MAX-WIDTH del container
  html = html.replace(/max-width:\s*850px/gi, 'max-width: 100%')
  
  console.log('Changes made:', changes)
  console.log('HTML length after:', html.length)
  console.log('First 800 chars after:', html.substring(0, 800))
  console.log('Contains font-size: 16px?', html.includes('font-size: 16px'))
  console.log('Contains font-size: 32px?', html.includes('font-size: 32px'))
  console.log('Contains line-height: 1.0?', html.includes('line-height: 1.0'))
