...
export function AgencyMemberForm({ agencyId }: AgencyMemberFormProps) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!res.ok || data.status === "rejected") {
        setStatus("error");
        setError(data.reason ?? "No se pudo agregar el miembro.");
        return;
      }
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 text-sm text-slate-200">
      ...
      <p className="mt-2 text-slate-400">Asigná roles estrictos por usuario.</p>
      <textarea
        className="mt-4 min-h-[180px] w-full rounded-xl border border-slate-800 bg-slate-950 p-4 text-xs text-slate-200"
        value={payload}
        onChange={(event) => setPayload(event.target.value)}
      />
      {status === "error" && <p className="mt-3 text-xs text-rose-300">{error}</p>}
      {status === "done" && <p className="mt-3 text-xs text-emerald-300">Miembro agregado.</p>}
      <button
        className="mt-4 rounded-xl bg-white px-4 py-3 text-sm font-semibold text-slate-950"
        onClick={submit}
        disabled={status === "loading"}
      >
        {status === "loading" ? "Agregando..." : "Agregar miembro"}
      </button>
    </div>
  );
}
