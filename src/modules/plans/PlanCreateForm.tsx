...
export function PlanCreateForm() {
  ...
  "priceCents": 4900,
  "interval": "monthly",
  "features": ["hosting", "redeploys", "support"],
  "isActive": true
}`);
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!res.ok || data.status === "rejected") {
        setStatus("error");
        setError(data.reason ?? "No se pudo crear el plan.");
        return;
      }
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 text-sm text-slate-200">
      ...
      <p className="mt-2 text-slate-400">Cargá el JSON y publicá el plan mensual.</p>
      <textarea
      ...
      {status === "done" && <p className="mt-3 text-xs text-emerald-300">Plan creado.</p>}
      <button
