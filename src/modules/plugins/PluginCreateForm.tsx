...
export function PluginCreateForm() {
  ...
  "slug": "seo-keywords",
  "name": "SEO Keywords",
  "description": "Agrega notas de keywords al delivery.",
  "version": "1.0.0",
  "entryPoint": "seo-keywords",
  "scope": "seo",
  "priceCents": 1900,
  "authorId": ""
}`);
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!res.ok || data.status === "rejected") {
        setStatus("error");
        setError(data.reason ?? "No se pudo crear el plugin.");
        return;
      }
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 text-sm text-slate-200">
      <h2 className="text-lg font-semibold">Registrar plugin</h2>
      <p className="mt-2 text-slate-400">Cargá el JSON y registrá el plugin.</p>
      <textarea
      ...
      {status === "done" && <p className="mt-3 text-xs text-emerald-300">Plugin creado.</p>}
      <button
        className="mt-4 rounded-xl bg-white px-4 py-3 text-sm font-semibold text-slate-950"
        onClick={submit}
        disabled={status === "loading"}
      >
        {status === "loading" ? "Registrando..." : "Registrar plugin"}
      </button>
    </div>
  );
}
